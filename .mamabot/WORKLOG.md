...
이 파일은 기존 plan.md의 긴 작업 기록을 보관하는 파일이다.
에이전트는 사용자가 명시적으로 요청하지 않는 한 이 파일을 자동으로 읽지 않는다.

MigratedAt: 2026-05-15T04:02:27.075Z
...
## 2026-05-16 13:23:17
- 인증형 CLI 실행/저장/대화창 연결/백그라운드 실행 흐름을 정리했다.
- Codex CLI는 codex exec + stdin prompt + workspace index 후보 기반으로 동작하도록 개선했다.
- AGENTS.md / PROJECT_INDEX.md를 토큰 절약형으로 정리했다.
- 검증 결과 Codex Review 토큰 사용량이 약 17,109로 감소했다.
- 남은 과제: suggest 모드 반영, reasoning effort 프로필화, Windows quoting 안정화.

<!-- MAMABOT_WORK_20260516_MODEL_ENV_REFRESH -->

## 2026-05-16 작업 완료 기록: 모델/API Key UX 및 실시간 모델 목록 개선

### 완료한 작업

1. **워크스페이스 인덱스 정확한 경로 우선순위 보정**
   - 프롬프트에 pp/api/agent/run/route.js처럼 정확한 파일 경로가 포함되면 해당 파일이 후보 1순위로 올라오도록 개선했다.
   - 엉뚱한 oute.js가 먼저 잡히는 문제를 줄였다.
   - 관련 커밋:  bfdcb6

2. **에이전트 실행 안전정책 기록 추가**
   - 실행 응답/run JSON에 safetyPolicy를 남기도록 개선했다.
   - suggest는 ead-only, uto/coding은 workspace-write로 정책이 보이게 했다.
   - dangerFullAccess: false, equiresPreBackup, equiresPostCheck 정보도 확인 가능하게 했다.
   - 관련 커밋: 7a5a4d

3. **실시간 OpenRouter 모델 뱃지 표시 추가**
   - /api/models/openrouter에서 OpenRouter 모델 목록을 실시간으로 가져오고 캐시한다.
   - 모델별로 FREE, 인기순위, 최신순, 64K+, 대용량 등 뱃지 정보를 붙일 수 있게 했다.
   - 무료 모델이 먼저 나오고, 인기순위/최신순/컨텍스트 기준으로 정렬되도록 개선했다.
   - 관련 커밋: ddddb7a

4. **API Key 저장 피드백 개선**
   - API Key 저장 후 메시지를 더 명확하게 표시한다.
   - 저장 후 Key 입력칸은 유지하고, Value 입력칸만 비우도록 개선했다.
   - 저장 완료 시 mamabot:models-refresh 이벤트를 발생시켜 모델 목록 자동 갱신의 기반을 만들었다.
   - 관련 커밋:  571632

5. **API Key 저장 후 모델 목록 자동 새로고침 연결**
   - AgentPanel.jsx가 mamabot:models-refresh 이벤트를 받아 OpenRouter 모델 목록을 자동 새로고침하도록 연결했다.
   - API Key 저장 직후 대시보드 모델 선택 목록이 최신 상태로 갱신될 수 있게 했다.
   - 관련 커밋: 7454aaa

6. **워크벤치 모델 목록 실시간 OpenRouter 우선 적용**
   - WorkbenchChatPanel.jsx에서 실시간 OpenRouter 모델 목록을 우선 사용하도록 개선했다.
   - 실시간 목록이 있으면 기존 하드코딩 무료 모델 목록은 숨기고, 실시간 목록이 없을 때만 fallback으로 사용한다.
   - Workbench도 mamabot:models-refresh 이벤트를 받아 모델 목록을 자동 새로고침한다.
   - 관련 커밋: e71449b

### 현재 기준 상태

- main 브랜치와 origin/main은 동기화된 상태다.
- API Key 저장 → 모델 목록 자동 새로고침 → Dashboard/Workbench 반영 흐름이 연결됐다.
- OpenRouter 모델 목록은 하드코딩이 아니라 /api/models/openrouter 실시간/캐시 기반으로 사용하는 방향으로 정리됐다.

### 다음 작업 후보

1. **모델 / 인증 화면 2차 UX 개선**
   - 환경변수 목록에서 저장됨 / 비어 있음 상태를 더 보기 좋게 표시한다.
   - Key 선택 버튼, 삭제 확인, 저장 상태 표시를 더 안정적으로 다듬는다.

2. **/api/workspace, /api/runtime 404 정리**
   - 현재 서버 로그에 남는 404 호출 원인을 확인하고 실제 route를 만들지, 호출부를 제거할지 결정한다.

3. **
pm run build 시 runtime/hermes/venv EACCES 문제 정리**
   - Next build가 untime/hermes/.../venv를 스캔하다가 막히는 문제를 정리한다.
   - untime, ackups, env 계열 폴더가 빌드 대상에 섞이지 않도록 방어한다.

4. **실시간 모델 뱃지 표시 강화**
   - Dashboard/Workbench에서 FREE, 인기, 최신, 대용량, Vision, 코딩추천 뱃지를 더 보기 좋게 표시한다.
   - 모델 검색/필터 UX를 개선한다.

...
F:\mamabot\PLAN.md 기준으로 이어서 작업하자. 현재 Hermes 실제 실행은 nvidia/nemotron-3-super-120b-a12b:free 모델로 성공했고, 다음 작업은 실행 결과 저장 / 최근 실행 목록 기능 추가야. PowerShell 명령어부터 줘.
...
- Mamabot 본체와 Hermes runtime은 F:\mamabot 내부에 유지.
...
- 전체 백업 수행 완료.
- 백업 위치:
  - F:\mamabot\backups\mamabot-full-20260511-212629
- Robocopy 결과:
  - 복사 파일: 약 77,255개
  - 복사 용량: 약 1.857GB
  - 실패: 0
...
### 컴포넌트 백업
- 주요 컴포넌트 수정 전 백업 위치:
  - F:\mamabot\backups\component-backups

### route 백업
- 주요 API route 수정 전 백업 위치:
  - F:\mamabot\backups\route-backups

## 3. 인증 상태

### OpenRouter API Key
- .env 저장 위치:
  - F:\mamabot\runtime\hermes\home\.env
- hermes status 기준:
...
- WSL 작업 경로:
  - /mnt/f/mamabot
- Hermes runtime 위치:
  - F:\mamabot\runtime\hermes\home
- Hermes Agent 설치 위치:
  - F:\mamabot\runtime\hermes\home\hermes-agent
- Mamabot Web UI:
  - http://localhost:3200
...
- 로그 파일:
  - F:\mamabot\runtime\hermes\logs\mamabot-agent-run.log
- 화면 안에서 로그 확인 가능.
- 자동갱신 ON/OFF 버튼 있음.
- 화면 비우기 버튼 있음.
...
- hardcoded old path such as F:\mamabot
...
<!-- MAMABOT_ROUTING_FIX_20260516_BEGIN -->

# Mamabot 실행 라우팅 1차 보정 - 2026-05-16

## 문제

Quick 실행 시 Claude Code / Codex CLI / Gemini CLI 같은 인증형 CLI Provider도 Direct API 경로로 내려가면서 OpenRouter 기본값을 타는 문제가 있었다.

또한 Gemini / Anthropic / OpenAI API Provider를 선택하더라도 해당 API Key가 없으면 조용히 OpenRouter로 fallback되는 구조가 있었다.
...
- Auth CLI Provider는 Quick 여부와 관계없이 실제 실행 시 CLI Runner를 타도록 수정했다.
- Dry-run에서는 CLI가 실제 실행되지 않도록 dryRun === false 조건을 유지했다.
- Quick Direct에서 route.js가 OpenRouter 전용 기본 모델을 강제 주입하지 않도록 수정했다.
- directModelClient에서 Gemini / Anthropic / OpenAI / DeepSeek direct 호출 실패 시 OpenRouter로 조용히 우회하던 fallback을 제거했다.
- OpenAI direct adapter를 추가했다.
- 지원하지 않는 Direct Provider는 명시적으로 실패하도록 했다.

## 검증 결과

확인된 결과:

- directModelClient import OK
- gemini-api 선택 시 GEMINI_API_KEY 또는 GOOGLE_API_KEY 없으면 실패
- anthropic 선택 시 ANTHROPIC_API_KEY 없으면 실패
- openai 선택 시 OPENAI_API_KEY 없으면 실패
- unknown provider는 OpenRouter로 우회하지 않고 unsupported provider로 실패

## 다음 작업

1. AgentPanel에서 Quick / Detailed / Agent / Coding / Review 개념을 UI상 분리한다.
2. Provider 선택 화면을 카드형으로 개선한다.
3. 선택한 Provider에 맞는 모델만 보이도록 모델 선택 UX를 정리한다.
4. 모델 등록 창을 Provider별 직접 등록 방식으로 재구성한다.
...
<!-- MAMABOT_AI_SETTINGS_PANEL_20260516_BEGIN -->
# AI ?? ?? 1? ?? - 2026-05-16

## ?? ??
- app/components/AISettingsPanel.jsx ??
- AppShell? models ?? AISettingsPanel? ??
- ?? ModelsPanel.jsx? ???? ?? ? ??? ?? ?? ??? ?? ???? ??
- Provider Sidebar ? API Key ?? ? ?? ??? ? ?? ?? ???? ? ???? ???? UI ??
...
- ?? ?????? API Key ??/?? ?? ??
- AgentPanel ?? ???? ????/?? ?? ???? ??
- OpenRouter ? Gemini/DeepSeek/GitHub Models ?? ?? API ??
<!-- MAMABOT_AI_SETTINGS_PANEL_20260516_END -->

<!-- MAMABOT_QUICK_SEARCH_V1_20260516_BEGIN -->
# Quick Search v1 - 2026-05-16

## ?? ??
- app/api/quick-search/route.js ??
- Workbench quick profile? /api/agent/run ?? /api/quick-search? ??
- Quick? workspace/index/file read/patch? ???? ??
- ??? provider/model? direct adapter? ?? ??? ??
- ?? ?? ??: short 180, normal 350, detailed 700
- ?? ?? ?? ??? intent? ?? ???? ??

## ?? ??
- Brave/Tavily/Search provider ??
- weather/finance/crypto/fx ?? fetch route ??
- evidence pack max token limiter ??
- Quick ??? source summary UI ??
<!-- MAMABOT_QUICK_SEARCH_V1_20260516_END -->